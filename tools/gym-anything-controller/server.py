"""
gym-anything-controller MCP server

Bridges Claude CLI to a gym-anything environment via the HTTP bridge.
Matches the exact same computer_use tool interface that gym-anything's
...
1920x1080 (gym-anything's native resolution) before sending to the bridge.

Requires env var:
    GA_BRIDGE_URL  e.g. http://127.0.0.1:8766
"""
...
BRIDGE_URL = os.environ.get("GA_BRIDGE_URL", "http://127.0.0.1:8766")

# gym-anything native resolution (all envs use 1920x1080)
NATIVE_WIDTH = 1920
NATIVE_HEIGHT = 1080

# Model sees screenshots at this resolution (matches gym-anything's ClaudeAgent)
MODEL_WIDTH = 1280
MODEL_HEIGHT = 720

mcp = FastMCP("gym-anything-controller")
...
def _scale_coords(x: int, y: int) -> list[int]:
    ...
    This matches gym-anything's convert_point_format_claude():
