...
You can manage all your conditional rules in one place via **Format > Conditional > Manage**, which lists every rule and the range it applies to. From there, **Edit** tweaks an existing rule and **Remove** deletes one outright — no hunting through menus. See `fig01.png` for the Manage Conditional Formatting dialog.
