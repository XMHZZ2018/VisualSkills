...
The Area dialog is shown with the Color tab selected, displaying buttons for None, Color, Gradient, Bitmap, Pattern, and Hatch along the top. On the left, a Colors section shows a Palette dropdown set to "standard" with a grid of color swatches below it. The center displays an Active color preview (a light blue, with R 114, G 159, B 207, Hex 729fcf), and the right shows a New color preview (a dark green, with R 22, G 111, B 55, Hex 166f37) along with a Pick button. A "Name" pop-up dialog is overlaid in the center, prompting "Please enter a name for the new color:" with a text field pre-filled with "Color 2" and Help, OK, and Cancel buttons. At the bottom left of the main dialog, Add and Delete buttons appear under a Custom Palette label.
...
The bottom section of the Tools > Options > LibreOffice > Advanced dialog is shown, containing an Optional Features group with two checkboxes, both checked: "Enable experimental features (may be unstable)" and "Enable macro recording (may be limited)."
