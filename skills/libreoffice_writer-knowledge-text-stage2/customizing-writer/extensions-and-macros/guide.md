...
The screenshot shows the Tools drop-down menu fully expanded in LibreOffice Writer, with the "Tools" label highlighted in the menu bar. The menu lists items in order from top to bottom: Spelling…, Automatic Spell Checking (with a checkbox, currently checked), Thesaurus…, Language, Word Count…, Accessibility Check…, Automatic Accessibility Checking (with an unchecked checkbox), AutoCorrect, AutoText…, ImageMap (grayed out), Redact, Auto-Redact, Heading Numbering…, Line Numbering…, Footnote/Endnote Settings…, Mail Merge Wizard…, Bibliography Database, Address Book Source…, Update, Protect Document, Calculate (grayed out), Sort… (grayed out), Macros, Development Tools (with an unchecked checkbox), XML Filter Settings…, Extensions…, Customize…, and Options… at the bottom. Several entries such as Language, AutoCorrect, Update, Protect Document, and Macros have submenu arrows indicating expandable submenus.
