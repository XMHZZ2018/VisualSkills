...
The Save As Template dialog contains an "Enter Template Name" text field at the top, followed by a "Select Template Category" list showing categories such as None, My Templates, Business Correspondence, draw, Other Business Documents, Personal Correspondence and Documents, Presentations, and Styles. Below the category list is a "Set as default template" checkbox, and the dialog has Help, Cancel, and Save buttons along the bottom.
...
The Templates dialog displays a search field on the left, filter dropdowns for document type (set to "Text Documents") and category (set to "All Categories"), and a Manage dropdown button in the upper-right corner. The main area shows thumbnail previews of available templates such as "LibreOffice_7.x_Template," "Modern business letter sans-serif," "Modern business letter serif," and "Businesscard with logo." The Manage dropdown menu is open, revealing options: New Category, Refresh, Reset Default Text Document, Import, and Extensions.
...
The File menu is shown expanded from the menu bar (which also includes Edit, View, Insert, and Format). It lists entries in order from top to bottom: New, Open…, Open Remote…, Recent Documents, Close, Wizards, Templates, Reload (greyed out), Versions…, Save, Save As…, Save Remote…, Save a Copy…, Save All (greyed out), Export…, Export As, Send, Preview in Web Browser, Print Preview, Print…, Printer Settings…, Properties…, Digital Signatures, and Exit LibreOffice.
