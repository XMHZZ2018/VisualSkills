...
The Define Name dialog has a "Name" text field at the top (here showing "ExamResults" as an example), a "Range or formula expression" field below it (showing an absolute range like `$Sheet1.$A$1:$H$11`), and a "Scope" dropdown set to "Document (Global)." An expandable "Range Options" section at the bottom offers checkboxes for Print range, Filter, Repeat column, and Repeat row, all unchecked by default. The dialog's action buttons are Help, Add (highlighted as the default), and Cancel.
...
The Validity dialog is shown with three tabs along the top: Criteria, Input Help, and Error Alert, with the Criteria tab currently active. The "Allow" dropdown is set to "Cell range" (highlighted in blue); below it are checkboxes for "Allow empty cells" (checked) and "Show selection list" (checked), with a nested unchecked "Sort entries ascending" option. A "Source" field with an adjacent shrink/expand button lets you specify a cell range or formula, and a help note below it explains that a valid source must be a contiguous selection of rows and columns or a formula resulting in an area or array. The bottom of the dialog has Help, Reset, OK (highlighted as default), and Cancel buttons.
...
The AutoFilter combo box is shown dropped down from column A (headed "Student") of a spreadsheet. At the top of the dropdown are Sort Ascending and Sort Descending options, followed by Top 10, Empty, and Not Empty filters. Below those are Text color and Background color filter options (with colored swatches—green, red, and yellow—visible to the right), then a "Standard Filter…" link for advanced criteria. A "Search items…" text field appears next, followed by an "All" checkbox (checked) with select-all and deselect-all buttons beside it. The scrollable list below shows individual borrower names—Andrew (highlighted in blue), Bethany, Charles, David, Emily, Ferdinand, Georgia, Haley—each with a checked checkbox. OK and Cancel buttons sit at the bottom of the dropdown.
...
The Tools menu is shown expanded from the menu bar. Its entries from top to bottom are: Spelling…, Automatic Spell Checking (checked), Thesaurus…, Language, AutoCorrect Options…, AutoInput (checked), ImageMap (greyed out), Redact, Auto-Redact, Goal Seek…, Solver…, Detective, Scenarios… (greyed out), Forms, Share Spreadsheet…, Protect Sheet… (with an unchecked checkbox), and Protect Spreadsheet Structure… (with additional items cut off below).
