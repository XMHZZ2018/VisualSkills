...
When you need help building a formula, open the Function Wizard via **Insert > Function** (or press *Ctrl+F2*). It lists all 500+ available functions organized by category — Mathematical, Statistical, Date&Time, Logical, and more. Pick a function and you'll see a description plus input fields for each argument, along with a live preview of the result. The Function Wizard dialog has two tabs — Functions and Structure — on the left side. Under the Functions tab, a Search field sits at the top, followed by a Category dropdown (set here to "Mathematical") and a scrollable Function list below it. The right side shows the selected function's name (e.g., "SUM"), a brief description ("Returns the sum of all arguments."), labeled argument input fields (Number 1 through Number 4, each with an fx button and a shrink/expand toggle), and a Formula preview area at the bottom displaying the formula being built (e.g., `=SUM()`) alongside a live Result readout. Along the bottom of the dialog are an Array checkbox, a Help button, Back/Next navigation buttons, and OK/Cancel buttons.
...
The Formula Bar is a narrow horizontal strip spanning the width of the window. On the far left is the Name Box displaying the current cell address "A1" with a text cursor and a small dropdown arrow (▼) to its right. After the Name Box, three small buttons appear in sequence: the fx button (Function Wizard), the Σ button with its own dropdown arrow (Select Function), and an equals sign (=) button that activates formula entry mode. The remainder of the bar to the right is the wide Input Line, an empty text field where cell contents and formulas are typed or displayed.
...
The Insert dropdown menu is shown expanded from the menu bar, positioned between the View and Format menus. It lists items in a single column from top to bottom: Image…, Chart…, Sparkline…, Pivot Table…, Media (with a submenu arrow), OLE Object (with a submenu arrow), Shape (with a submenu arrow), then a separator line, followed by Function…, Named Range or Expression…, then another group starting with a checkbox-style Text Box entry, Comment, Fontwork…, then Hyperlink… (also with a checkbox), Special Character…, and Formatting Mark (with a submenu arrow). The menu items are displayed in a clean, standard system-styled dropdown against the spreadsheet background.
