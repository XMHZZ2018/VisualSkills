...
The spreadsheet shows the grade category labels (such as Homework, Midterm, and Final) in column A with their decimal weights in column B and a student's scores in column C; the SUMPRODUCT weighted-average formula result appears below the data, and a separate column to the right lists hypothetical exam scores alongside an empty adjacent column reserved for the Multiple Operations output. The "Multiple Operations" dialog is a small window with two input fields: the **Formulas** field at the top is set to the cell reference of the weighted-average SUMPRODUCT formula, and the **Column input cell** field below it is set to the cell containing the original exam score that the formula depends on; **OK**, **Cancel**, and **Help** buttons appear along the right side, and a shrink button next to each field allows clicking directly on a cell in the sheet.
...
The spreadsheet displays a two-dimensional what-if table: one set of hypothetical scores runs down a column on the left and a second set runs across a row at the top, with the weighted-average formula cell sitting at the intersection of the row and column headers. The body of the table is filled with computed weighted averages for every combination of the two variables, forming a grid that lets you see at a glance how changing either score shifts the final grade.
...
The "Goal Seek" dialog is a compact window with three fields stacked vertically: **Formula cell** at the top (pre-filled with the address of the selected weighted-average cell), **Target value** in the middle (where you type the desired grade, such as 85), and **Variable cell** at the bottom (set to the cell reference of the final-exam score that Calc will solve for). Each field has a shrink button for on-sheet cell selection, and **OK**, **Cancel**, and **Help** buttons line the right edge of the dialog.
...
The Formula bar spans the full width of the spreadsheet window directly beneath the formatting toolbar. On the far left is the Name Box showing the active cell reference (e.g. "D7"); next to it are three small buttons — the Function Wizard button labeled "fx," the Sum/Select Function button labeled with a sigma (Σ), and the formula-mode equals-sign button (=). To the right of these buttons, the wide Input Line stretches across the remainder of the bar, displaying the raw content of the active cell (in this case a SUMPRODUCT formula). A small chevron arrow at the far right edge of the Input Line allows expanding the bar to multiple lines for editing long formulas.
...
The Sheet menu is shown fully expanded, revealing a long vertical list of menu items organized into logical groups separated by horizontal dividers. The top section contains cell and row/column insertion commands (Insert Cells, Insert Rows, Insert Columns, Insert Page Break), followed by their deletion counterparts and Clear Cells. The middle section includes Fill Cells, Named Ranges and Expressions, and Cell Comments — each with a right-arrow indicating a submenu. The lower section lists sheet-management commands such as Insert Sheet, Delete Sheet, Rename Sheet, Hide/Show Sheet, Move or Copy Sheet, Navigate, Sheet Tab Color, Sheet Events, and Right-To-Left.

The Data menu is shown fully expanded as a vertical dropdown. At the top are Sort, Sort Ascending, and Sort Descending, followed by AutoFilter and More Filters with a submenu arrow. The middle section includes Define Range, Select Range, Refresh Range, and the Pivot Table submenu. Below that are Calculate, Validity, Subtotals, Form, Streams, XML Source, and — highlighted or notable — **Multiple Operations…**, which is the entry point for what-if scenario tables. The bottom section contains Text to Columns, Consolidate, Group and Outline (with submenu arrow), and a Statistics submenu listing thirteen statistical analysis tools.
...
The Tools menu is shown fully expanded as a vertical dropdown. It begins with Spelling (F7), Automatic Spell Checking, Thesaurus, Language, AutoCorrect Options, and AutoInput. A divider separates these from ImageMap (greyed out), Redact, and Auto-Redact. The next group contains the analysis tools: **Goal Seek…** and **Solver…**, followed by the Detective submenu for formula auditing and Scenarios (greyed out). Below another divider are Forms (with a submenu arrow), Share Spreadsheet, Protect Sheet, and Protect Spreadsheet Structure. The bottom section lists Macros (with submenu), Development Tools, XML Filter Settings, Extensions (Ctrl+Alt+E), Customize, and Options (Alt+F12).
