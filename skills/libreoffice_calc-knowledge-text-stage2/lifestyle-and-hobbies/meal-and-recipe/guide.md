...
The spreadsheet shows two views of a simple calculation in columns A through C. In the top view, cell B3 contains the formula `=15+46` as typed by the user. In the bottom view, the same cell B3 now displays the computed result, 61, with the cell selected and showing a fill handle in its lower-right corner.
...
The "Multiple operations" dialog has the title bar "Multiple operations" and contains a "Default Settings" section with three text input fields, each accompanied by a shrink/expand button on the right: "Formulas:", "Row input cell:", and "Column input cell:". At the bottom of the dialog are three buttons: "Help" on the left, and "OK" (highlighted with a blue border) and "Cancel" on the right.
...
The spreadsheet shows the results of a Multiple Operations run with one formula and one variable. Columns A and B on the left hold the base model: Unit sale price ($10), Unit cost ($2), Fixed annual cost ($10,000), Quantity sold (2,000), and Profit ($6,000). Columns D and E on the right display the what-if results table, with "Quantity" in D1 listing values from 500 to 5000 in increments of 500, and "Profit" in E1 showing the corresponding calculated profit for each quantity — ranging from -$6,000 (at 500 units) up to $30,000 (at 5,000 units). The formula bar for the selected cell E11 displays `=MULTIPLE.OPERATIONS(B$5,$B$4,$D11)`.
...
The Sheet drop-down menu is shown expanded from the menu bar (which reads Format, Styles, Sheet, Data, Tools, Window). The menu lists items in order from top to bottom: Insert Cells…, Insert Rows, Insert Columns, Insert Page Break, Delete Cells…, Delete Rows, Delete Columns, Delete Page Break, Insert Sheet…, Insert Sheet at End…, Insert Sheet from File…, External Links…, Delete Sheet… (greyed out), Clear Cells…, Cycle Cell Reference Types, Fill Cells, Named Ranges and Expressions, Cell Comments, Rename Sheet…, and the list continues below the visible area.

The Data drop-down menu is shown expanded from the menu bar. It lists items from top to bottom: Sort…, Sort Ascending, Sort Descending, AutoFilter (with a checkbox to its left, currently unchecked), More Filters, Define Range…, Select Range…, Refresh Range (greyed out), Pivot Table, Calculate, Validity…, Subtotals…, Form…, Streams…, and the list continues below the visible area.
