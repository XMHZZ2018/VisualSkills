...
The Sort dialog's Options tab displays a list of checkboxes under a "Sort Options" heading: Case sensitive, Range contains column labels (checked), Include formats (checked), Enable natural sort, Include boundary column(s) containing only comments, Include boundary column(s) containing only images (checked), Copy sort results to (with a greyed-out destination dropdown), and Custom sort order (with a dropdown showing "Sun,Mon,Tue,Wed,Thu,Fri,Sat"). Below these are Language and Options dropdowns (set to "Default - English (UK)"), and a Direction section with radio buttons for "Top to bottom (sort rows)" (selected) and "Left to right (sort columns)". The dialog has Help, Reset, OK, and Cancel buttons along the bottom.
...
The Function Wizard dialog is shown with the Functions tab active. On the left side, the Category dropdown is set to "Mathematical" and the Function list below it shows SUBTOTAL highlighted in blue among neighboring functions like SQRT, SQRTPI, SUM, SUMIF, SUMIFS, and others. On the right side, the dialog displays the function name "SUBTOTAL" with a Function result of "$408,765.00" and a description reading "Calculates subtotals in a spreadsheet." Two input fields are shown: Function with a value of "9" and Range with a value of "B2:B157". At the bottom, the Formula field shows "=SUBTOTAL(9,B2:B157)" with a Result of "$408,765.00". The dialog has an Array checkbox, and Help, Back, Next, OK, and Cancel buttons.
...
The Subtotals dialog has four tabs along the top: 1st Group (currently selected), 2nd Group, 3rd Group, and Options. The "Group by" dropdown is set to "Employee". On the left, a "Calculate subtotals for" checklist shows columns Date, Sales Value (checked), Category, Region, and Employee. On the right, a "Use function" list shows Sum (highlighted), Count, Average, Max, Min, Product, Count (numbers only), StDev (Sample), StDevP (Population), Var (Sample), and VarP (Population). The dialog has Help, OK, Cancel, Reset, and Remove buttons along the bottom.
...
The Sheet menu is shown expanded from the menu bar (between Styles and Data). It lists, from top to bottom: Insert Cells…, Insert Rows, Insert Columns, Insert Page Break, then a separator, followed by Delete Cells…, Delete Rows, Delete Columns, Delete Page Break, another separator, then Insert Sheet…, Insert Sheet at End…, Insert Sheet from File…, External Links…, and Delete Sheet… (greyed out). After another separator: Clear Cells…, Cycle Cell Reference Types, Fill Cells, Named Ranges and Expressions, Cell Comments, and Rename Sheet… with additional items partially visible below.

The Data menu is shown expanded from the menu bar (between Sheet and Tools). It lists, from top to bottom: Sort…, Sort Ascending, Sort Descending, then a checkbox for AutoFilter, followed by More Filters. After a separator: Define Range…, Select Range…, Refresh Range (greyed out). Then: Pivot Table, Calculate, Validity…, Subtotals…, Form…, and Streams… with additional items partially visible below.
