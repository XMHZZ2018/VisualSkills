...
The Text Import dialog is divided into sections from top to bottom: the **Import** section at the top contains the Character Set dropdown and Language dropdown; below that, **Separator Options** offers radio buttons for Separated by (with checkboxes for Tab, Comma, Semicolon, Space, and a custom Other field) or Fixed width, along with checkboxes for Merge delimiters and Trim spaces, plus a String delimiter field. The lower half of the dialog displays the **Fields** preview — a columnar table showing how the data will be split, with a Column type dropdown above it that lets you set each column to Standard, Text, Date (with format codes like DMY, MDY, YMD), or Hide. OK, Cancel, and Help buttons run along the bottom.
...
The Export Text File dialog shows three main fields: a **Character set** dropdown (typically set to Unicode UTF-8), a **Field delimiter** dropdown (defaulting to a comma), and a **String delimiter** dropdown (defaulting to a double-quote character). Below these, checkboxes allow you to toggle options such as **Save cell content as shown** and **Save cell formulas instead of calculated values**, as well as **Quote all text cells** and **Fixed column width**. The dialog includes OK, Cancel, and Help buttons at the bottom.
...
The Pivot Table Layout dialog is organized into several drag-and-drop zones arranged around a central area. At the top-left, the **Available Fields** list box shows all column names from the source data. To the right and below it are four target areas displayed as labeled boxes: **Filters** across the top, **Column Fields** along the upper portion, **Row Fields** down the left side, and **Data Fields** in the center. Fields that have been assigned appear as buttons within their respective target areas. Below the layout zones, an expandable **Options** section provides checkboxes for Total columns, Total rows, Add filter, Drill to detail, and Ignore empty rows. A **Source and Destination** section lets you specify the result location. OK, Cancel, and Help buttons sit along the bottom-right of the dialog.
...
The dialog window is titled "PDF Options" and displays six tabs across the top: General, Initial View, User Interface, Links, Security, and Digital Signatures. The General tab is selected, showing a Range section with radio buttons for All, Pages (with a text field), and Selection/Selected sheet(s), followed by a "View PDF after export" checkbox. Below that, an Images section offers radio buttons for Lossless compression and JPEG compression (with a Quality slider set to 90%), plus a "Reduce image resolution" checkbox with a DPI dropdown set to 300. A Watermark section, General settings checkboxes (including Hybrid PDF, Archival PDF/A, Tagged PDF, and others), and a Structure section with checkboxes for Export outlines, Whole sheet export, and related options fill the remainder of the tab. Export and Cancel buttons appear at the bottom-right.
...
The File menu is shown fully expanded as a vertical dropdown from the menu bar. Menu items are listed top to bottom: New (with a submenu arrow), Open… (Ctrl+O), Open Remote…, Recent Documents (submenu arrow), Close, Wizards (submenu arrow), Templates (submenu arrow), Reload (greyed out), Versions… (greyed out), then a separator. Next come Save (Ctrl+S), Save As… (Shift+Ctrl+S), Save Remote…, Save a Copy…, Save All, followed by another separator. Export…, Export as PDF… appear next, then Send (with a submenu arrow). After another separator: Preview in Web Browser, Print Preview (Shift+Ctrl+O), Print… (Ctrl+P), Printer Settings…, then Properties…, Digital Signatures (submenu arrow), and finally Exit LibreOffice (Ctrl+Q) at the bottom. Each item shows its keyboard shortcut on the right side where applicable.
...
The Insert menu is shown fully expanded as a vertical dropdown. Items listed from top to bottom include: Image…, Chart…, Sparkline…, Pivot Table… (with a submenu arrow), then a separator. Media (submenu arrow), OLE Object (submenu arrow), Shape (submenu arrow) follow, then another separator. Function… (Ctrl+F2), Named Range or Expression… appear next. After a separator: Text Box (shown as a checkbox toggle), Comment (Ctrl+Alt+C), Fontwork…, Hyperlink… (Ctrl+K), Special Character…, Formatting Mark (submenu arrow). Then Date (Ctrl+;), Time (Shift+Ctrl+;), Field (submenu arrow), Headers and Footers…, and Form Control (submenu arrow) near the bottom, ending with Signature Line….
...
The Sheet menu is shown fully expanded as a vertical dropdown. At the top are structural cell operations: Insert Cells… (Ctrl++), Insert Rows (submenu arrow), Insert Columns (submenu arrow), Insert Page Break (submenu arrow), followed by their corresponding Delete counterparts and Clear Cells… (Backspace). Cycle Cell Reference Types (F4) appears next. After a separator, Fill Cells (submenu arrow) and Named Ranges and Expressions (submenu arrow) are listed, then Cell Comments (submenu arrow). A final section contains sheet management items: Insert Sheet…, Insert Sheet at End…, Insert Sheet from File…, External Links…, Delete Sheet… (greyed out), Rename Sheet…, Hide Sheet, Show Sheet…, Move or Copy Sheet…, Duplicate Sheet, Navigate (submenu arrow), Sheet Tab Color…, Sheet Events…, and Right-To-Left.

The Data menu is shown fully expanded as a vertical dropdown. Items from top to bottom include: Sort…, Sort Ascending, Sort Descending, then a separator. AutoFilter (Shift+Ctrl+L), More Filters (submenu arrow), followed by a separator. Define Range…, Select Range…, Refresh Range appear next. After a separator: Pivot Table (with a submenu arrow leading to Insert or Edit…, Refresh, and Delete). Then Calculate (submenu arrow), Validity…, Subtotals…, followed by a separator. Form…, Streams…, XML Source…, Multiple Operations…, Text to Columns…, Consolidate… come next, then another separator. Group and Outline (submenu arrow) and Statistics (submenu arrow) close out the menu.
