...
The Validity dialog is open with the Criteria tab selected. The Allow dropdown is set to "Date," the Operator dropdown shows "greater than," and the Minimum field contains a date value. The dialog also displays the Input Help and Error Alert tabs along the top, with OK, Cancel, and Reset buttons at the bottom.
...
The Validity dialog is shown with the Input Help tab active. At the top is a checkbox labeled "Show input help when cell is selected," which is checked. Below it are two fields: a Title text box containing a short heading and an Input help text area with a longer descriptive message explaining expected input. The Criteria and Error Alert tabs are also visible along the top of the dialog.
...
The LibreOffice Options dialog is open to the LibreOffice Calc > Calculate page. In the Formulas Wildcards section, the "Enable wildcards in formulas" radio button is selected (other options include "Enable regular expressions in formulas" and "No wildcards or regular expressions in formulas"). Additional settings for iterative references, date handling, and general calculation precision are visible in the same panel.
...
The Format Cells dialog is displayed with the Numbers tab active. On the left side is a Category list showing entries such as Number, Percent, Currency, Date, Time, and others. The center panel shows a Format list with sample number patterns, and the right side has options including Decimal places and Leading zeroes spinners, a Thousands separator checkbox, and a Negative numbers red checkbox. At the bottom is a Format Code text field with a preview area above it, and the Language dropdown is set to "Default - English (USA)."
...
The Sheet menu is fully expanded, showing groups of items including Insert Cells, Insert Rows, Insert Columns, Insert Page Break at the top, followed by Delete Cells, Delete Rows, Delete Columns, and Delete Page Break. Further down are Fill Cells, Named Ranges and Expressions, and Cell Comments submenus. The lower section lists sheet management commands: Insert Sheet, Delete Sheet, Rename Sheet, Hide Sheet, Show Sheet, Move or Copy Sheet, Duplicate Sheet, Navigate, Sheet Tab Color, Sheet Events, and Right-To-Left.

The Data menu is fully expanded, displaying Sort, Sort Ascending, Sort Descending, and AutoFilter at the top. Below are More Filters, Define Range, Select Range, and Refresh Range. The middle section shows Pivot Table, Calculate, Validity (the entry point for data validation), and Subtotals. The lower portion includes Form, Streams, XML Source, Multiple Operations, Text to Columns, Consolidate, Group and Outline, and Statistics.
...
The Tools menu is fully expanded, showing Spelling, Automatic Spell Checking, Thesaurus, and Language at the top. Below are AutoCorrect Options and AutoInput, followed by greyed-out ImageMap, Redact, and Auto-Redact entries. The middle section lists Goal Seek, Solver, and the Detective submenu (which contains Trace Precedents, Trace Dependents, Remove traces, Trace Error, Refresh Traces, Fill Mode, AutoRefresh, and Mark Invalid Data). The lower portion shows Scenarios, Forms, Share Spreadsheet, Protect Sheet, Protect Spreadsheet Structure, Macros, Development Tools, XML Filter Settings, Extensions, Customize, and Options at the very bottom.
