...
The screenshot shows two views of a LibreOffice Calc spreadsheet with columns A, B, and C visible. In the top view, cell B3 is selected and displays the formula `=15+46` being entered. In the bottom view, the formula has been evaluated and cell B3 now shows the computed result, 61, with the cell outlined in a thick black selection border and the fill handle visible at its lower-right corner.
...
The screenshot illustrates relative references across two spreadsheet views, each showing columns A through D. In the top view, columns B and C contain values in rows 3 and 4 (B3: 15, C3: 4, B4: 46, C4: 11), with cell B5 selected showing the sum 61; a red outline on cell C5 indicates the cell is empty and would receive a copied formula. In the bottom view, the same data appears but now the formula has been copied: B5 still shows 61, and C5 displays 15, demonstrating how relative references shift when a formula is dragged from one cell to an adjacent one, with the selection handle visible at the bottom-right of D5.
...
The screenshot shows the Format Cells dialog open with the Numbers tab active and highlighted in blue. The dialog displays a Category list on the left containing entries such as All, User-defined, Number (currently selected and highlighted in blue), Percent, Currency, Date, Time, Scientific, Fraction, and Boolean Value. To the right is a Format list showing preview examples like General, -1235, -1234.57, -1,235, -1,234.57, (1,235), (1,234.57), "one hundred", and "One hundred". At the bottom, an Options section is partially visible with a "Decimal places" spinner and a "Negative" checkbox. The dialog's tab bar along the top shows the tabs Numbers, Font, Alignment, Borders, and partially visible Background tabs.
...
The screenshot shows the Formatting toolbar as a single horizontal icon row. From left to right it contains: a Font Name dropdown (currently displaying "Liberation Sans" with a blue highlight), a Font Size dropdown (showing "10 pt"), then Bold (B), Italic (I), and Underline (U) toggle buttons. Next are the Font Color and Background Color split-buttons (the Font Color button shows a red "A" indicator). Following those are horizontal alignment buttons (Align Left, Align Center, Align Right), vertical alignment buttons (Align Top, Center Vertically, Align Bottom), and a Wrap Text toggle. To the right are Merge Cells icons, then number format buttons including a currency icon, a percent (%) button, and a decimal places (0.0) button, along with additional formatting controls at the far right.
