...
The screenshot shows two small spreadsheet grids side by side, each with columns A, B, and C. In the top grid, cell B3 contains the formula `=15+46` being entered, with the cursor visible. In the bottom grid, cell B3 displays the computed result `61`, and cell B4 is selected, showing the small square fill handle at its bottom-right corner ready for dragging.
...
The screenshot shows the Fill Series dialog box, titled "Fill Series." On the left is a Direction group with radio buttons for Down (currently selected), Right, Up, and Left. In the center is a Series Type group with radio buttons for Linear (currently selected), Growth, Date, and AutoFill. On the right is a Time Unit group with greyed-out radio buttons for Day, Weekday, Month, and Year. Below these groups are three text fields: Start value (set to "4"), End value (empty), and Increment (set to "-1"). At the bottom are Help, OK (highlighted with a blue border), and Cancel buttons.
...
The screenshot shows the Options dialog at the path "Options – LibreOffice Calc – Sort Lists." The left panel displays a tree of settings categories with "Sort Lists" highlighted under the expanded "LibreOffice Calc" node, which also shows General, Defaults, View, Calculate, Formula, Changes, Compatibility, Grid, and Print entries. Below that are collapsed nodes for LibreOffice Base, Charts, and Internet. The center of the dialog has a "Lists" pane showing several built-in sort lists, with "Sun,Mon,Tue,Wed,Thu,Fri,Sat" selected (highlighted in blue). To its right, the "Entries" pane displays the individual entries of that selected list — Sun, Mon, Tue, Wed, Thu, Fri, Sat — each on its own line. On the far right are New, Add, and Delete buttons. At the bottom is a "Copy list from" field containing the cell range "$Sheet2.$A$1:$A$13" with a Copy button beside it. The dialog's footer has Help, Reset, Apply, OK (blue-bordered), and Cancel buttons.
...
The Tools menu is shown expanded from the menu bar. It lists, from top to bottom: Spelling…, Automatic Spell Checking (checked), Thesaurus…, Language, AutoCorrect Options…, then a separator, AutoInput (checked), ImageMap (greyed out), Redact, Auto-Redact, then a separator, Goal Seek…, Solver…, Detective, Scenarios… (greyed out), then a separator, Forms, Share Spreadsheet…, Protect Sheet… (with an unchecked checkbox), Protect Spreadsheet Structure (with an unchecked checkbox), and the list continues below.
