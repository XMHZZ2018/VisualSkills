...
The Conditional Formatting dialog is titled "Conditional Formatting for A1:F8" and contains a "Conditions" section with a "Condition 1" block. This block has three dropdown fields in a row: the first is set to "Cell value," the second to "is equal to," and the third is an empty text input for the comparison value. Below that is an "Apply Style" dropdown set to "Accent," with a preview box to its right showing sample text "Lorem ipsum" in the chosen style. An "Enter a value" text area sits below for formula-based conditions. At the bottom of the conditions area are four buttons: Add, Delete, Up, and Down. A "Cell Range" section at the very bottom shows the Range field set to "A1:F8" with a shrink/expand button beside it, and the dialog closes with Help, OK, and Cancel buttons.
...
The Conditional Formatting dialog for Color Scale is also titled "Conditional Formatting for A1:F8." Inside the "Condition 1" block, two dropdowns at the top are set to "All Cells" and "Color Scale (3 Entries)." Below these are three columns labeled Minimum, (middle, unlabeled), and Maximum. Each column has a type dropdown — set to "Min," "Percentile," and "Max" respectively — a value field (the middle one shows "50"), and a color picker. The three color pickers are set to Red (left/minimum), Yellow (center/percentile), and Green (right/maximum). The same Add, Delete, Up, and Down buttons appear below the condition block, and the Cell Range section at the bottom shows "A1:F8" with Help, OK, and Cancel buttons.
...
The Goal Seek dialog is a compact window titled "Goal Seek" with a "Default Settings" section containing three fields stacked vertically: "Formula cell" (with a shrink/expand button for range selection), "Target value" (a plain text input), and "Variable cell" (also with a shrink/expand button). To the right of these fields are three buttons arranged vertically: OK (highlighted with a blue border as the default action), Cancel, and Help.
...
The Format menu is shown expanded from the menu bar. Its entries from top to bottom are: Text, Align Text, Number Format, Clone Formatting (with a checkbox icon), Clear Direct Formatting, Cells..., Rows, Columns, Merge and Unmerge C... (truncated), Character... (greyed out), Paragraph... (greyed out), Page Style..., Print Ranges, Conditional, AutoFormat Styles... (greyed out), Spreadsheet Theme, Theme..., Image, Chart, and additional items cut off below. The menu appears as a standard vertical dropdown against the spreadsheet background.
...
The Sheet menu is shown expanded from the menu bar. Its entries from top to bottom are: Insert Cells..., Insert Rows, Insert Columns, Insert Page Break, then a separator, followed by Delete Cells..., Delete Rows, Delete Columns, Delete Page Break, then another separator, followed by Insert Sheet..., Insert Sheet at End..., Insert Sheet from File..., External Links..., Delete Sheet... (greyed out), then a separator, followed by Clear Cells..., Cycle Cell Reference Types, Fill Cells, Named Ranges and Expressions (truncated), Cell Comments, Rename Sheet..., and the list continues below the visible area with Hide Sheet partially visible.

The Data menu is shown expanded from the menu bar. Its entries from top to bottom are: Sort..., Sort Ascending, Sort Descending, then a separator, followed by AutoFilter (with a checkbox icon), More Filters, then a separator, followed by Define Range..., Select Range..., Refresh Range (greyed out), then a separator, followed by Pivot Table, Calculate, Validity..., Subtotals..., Form..., Streams..., and the list continues below the visible area.
...
The Tools menu is shown expanded from the menu bar. Its entries from top to bottom are: Spelling..., Automatic Spell C... (truncated, with a checked checkbox icon indicating it is enabled), Thesaurus... (greyed out), Language, AutoCorrect Options (truncated), AutoInput (with a checked checkbox icon indicating it is enabled), ImageMap (greyed out), Redact, Auto-Redact, Goal Seek..., Solver..., Detective, Scenarios... (greyed out), Forms, Share Spreadsheet (truncated), Protect Sheet... (with an unchecked checkbox icon), Protect Spreadsh... (truncated, with an unchecked checkbox icon), and the list continues below the visible area.
