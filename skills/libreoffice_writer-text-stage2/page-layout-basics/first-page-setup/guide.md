...
The Page Style: Default Style dialog is shown with the **Footer** tab selected. Across the top are tabs for Organizer, Page, Area, Transparency, Header, Footer, Borders, Columns, and Footnote. In the Footer section, the **Footer on** checkbox is checked, the **Same content on left and right pages** checkbox is unchecked, and the **Same content on first page** checkbox is also unchecked, allowing independent footer content on the first page.
...
The Page Style: First Page dialog is shown with the **Organizer** tab selected. The Style section contains four fields: **Name** is set to "First Page," **Next style** is set to "Default Style" via a dropdown (highlighted in blue), **Inherit from** is empty, and **Category** is set to "Custom Styles." An **Edit Style** button appears beside each of the Next style and Inherit from fields.
...
The Title Page dialog is divided into three sections. The **Make Title Pages** section offers two radio buttons — "Converting existing pages to title pages" (selected) and "Insert new title pages" — followed by a **Number of title pages** spinner set to 1, and a **Place title pages at** option with "Document start" selected or a specific page number (set to 14). The **Page Numbering** section has a checked **Reset page numbering after title pages** checkbox with a page number spinner set to 1, and an unchecked **Set page number for first title page** checkbox with its own spinner. The **Edit Page Properties** section shows a **Style** dropdown set to "First Page" with an **Edit…** button beside it. Help, OK, and Cancel buttons appear at the bottom.
...
The Format menu is shown open in the Writer menu bar. It lists the following items top to bottom: Text, Spacing, Align Text, Clone Formatting, Clear Direct Formatting, Spotlight, then a separator, followed by Character…, Paragraph…, Lists, Bullets and Numbering…, Theme…, then another separator, Page Style…, Title Page…, Columns…, Comments (grayed out), Watermark…, Sections… (grayed out), then a separator, and context-sensitive entries: Image, Text Box and Shape, Frame and Object, Name… (grayed out), Alt Text… (grayed out), Anchor, Wrap, Arrange, Rotate or Flip, and Group.
