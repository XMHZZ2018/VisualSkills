...
The Options section of the Paragraph dialog's Text Flow tab shows four controls stacked vertically: a "Do not split paragraph" checkbox (unchecked), a "Keep with next paragraph" checkbox (unchecked), an "Orphan control" checkbox (checked, with a spinner set to 2 lines), and a "Widow control" checkbox (checked, with a spinner also set to 2 lines).
...
The Insert drop-down menu is shown expanded from the menu bar. It lists items in order from top to bottom: Page Break, More Breaks, then a separator, followed by Image…, Chart…, Media, OLE Object, Shape, Section…, Text from File…, Text Box, Comment, Frame, Fontwork…, Caption… (grayed out), then a separator, Hyperlink…, Bookmark…, Cross-reference…, then a separator, Special Character…, Formatting Mark, Horizontal Line, then a separator, Footnote and Endnote, Table of Contents and Index, Page Number…, Field, Header and Footer, then a separator, Envelope…, and Signature Line… at the bottom. Several entries such as More Breaks, Media, OLE Object, Shape, Frame, Formatting Mark, Footnote and Endnote, Table of Contents and Index, Field, and Header and Footer have right-pointing arrows indicating submenus.
