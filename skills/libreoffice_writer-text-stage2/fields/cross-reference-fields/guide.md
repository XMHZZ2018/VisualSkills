...
The Fields dialog is shown with the **Cross-references** tab selected (other tabs visible are Document, Functions, DocInformation, Variables, and Database). On the left, the **Type** list contains Set Reference, Insert Reference, Headings (currently highlighted), Numbered Paragraphs, Figure, Bookmarks, and Footnotes. The right side shows a **Selection** list with a filter box at the top, populated with document headings such as "Introduction," "Quick and easy field entry," and "Using automatic cross-references." Below the Type list is the **Insert reference to** list offering options including Page, Chapter, Reference (currently selected, shown with a dashed highlight), Above/Below, As Page Style, Number, Number (no context), and Number (full context). At the bottom of the dialog are **Name** and **Value** fields (Name shows "Copyright"), and **Help**, **Close**, and **Insert** buttons.
...
The screenshot shows the **Insert** dropdown menu open in the LibreOffice Writer menu bar (with View, Format, Styles, and Table visible to its right). The menu lists items top to bottom in this order: Page Break, More Breaks, Image…, Chart…, Media, OLE Object, Shape, Section…, Text from File…, Text Box, Comment, Frame, Fontwork…, Caption… (grayed out), then a separator, followed by Hyperlink…, Bookmark…, Cross-reference…, another separator, Special Character…, Formatting Mark, Horizontal Line, Footnote and Endnote, Table of Contents and Index, Page Number…, Field, Header and Footer, Envelope…, and Signature Line…. Small checkbox icons appear next to Hyperlink and Horizontal Line entries.
