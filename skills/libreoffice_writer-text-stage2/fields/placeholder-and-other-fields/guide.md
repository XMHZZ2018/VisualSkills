...
The Fields dialog is shown with the **Functions** tab selected (other tabs visible are DocInformation, Variables, Database, Document, and Cross-references). In the **Type** list on the left, "Placeholder" is highlighted among entries such as Conditional text, Input list, Input field, Execute macro, Combine characters, Hidden text, and Hidden Paragraph. The **Format** list in the middle shows the five placeholder subtypes — Text, Table, Frame, Image (currently highlighted), and Object. To the right are the **Placeholder** and **Reference** text input fields, with a **Macro…** button at the bottom right. Behind the dialog, the document shows a placeholder rendered as `<Logo>` in angled brackets with a tooltip reading "Insert logo here."
...
The Insert dropdown menu is shown open from the menu bar (between View and Format). It displays a single-column list of commands in the following order from top to bottom: Page Break, More Breaks, Image…, Chart…, Media, OLE Object, Shape, Section…, Text from File…, Text Box, Comment, Frame, Fontwork…, Caption… (grayed out), Hyperlink…, Bookmark…, Cross-reference…, Special Character…, Formatting Mark, Horizontal Line, Footnote and Endnote, Table of Contents and Index, Page Number…, Field, Header and Footer, Envelope…, and Signature Line…. Several items have submenu arrows, and separator lines group related commands into logical sections.
