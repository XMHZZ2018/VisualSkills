...
The screenshot shows the Table drop-down menu open from the Writer menu bar. It is a single-column menu listing items from top to bottom: Insert Table…, Insert, Delete, Select, and Size (each with a submenu arrow), followed by a separator and then Merge Cells, Split Cells…, Merge Table, and Split Table…. Below another separator are Protect Cells and Unprotect Cells, then AutoFormat Styles… and Number Format…. A checkbox item labeled Number Recognition appears next, followed by Header Rows Repeat Across Pages and Row to Break Across Pages (both partially truncated). The menu concludes with Convert (submenu arrow), Edit Formula, Sort…, and Properties…. Several items such as Merge Cells, Split Cells…, and Protect Cells appear greyed out, indicating the cursor is not currently inside a table.
