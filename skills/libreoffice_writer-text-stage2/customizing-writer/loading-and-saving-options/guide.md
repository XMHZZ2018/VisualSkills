...
The Load/Save – General options page is divided into three sections. At the top, the **Load** section has two checkboxes, both enabled by default: "Load user-specific settings with the document" and "Load printer settings with the document." Below that, the **Save** section contains checkboxes for "Save AutoRecovery information every" (enabled, with a spinner set to 10 minutes), "Edit document properties before saving" (unchecked), "Always create backup copy" (unchecked), "Save URLs relative to file system" (checked), and "Save URLs relative to internet" (checked). At the bottom, the **Default File Format and ODF Settings** section provides three drop-down menus: "ODF format version" set to "1.3 Extended (recommended)," "Document type" set to "Text document," and "Always save as" set to "ODF Text Document (*.odt)," followed by an unchecked "Warn when not saving in ODF or default format" checkbox.
...
The Tools drop-down menu is shown open from the Writer menu bar. It is a single-column list of items separated by horizontal dividers into logical groups. The top group contains Spelling…, Automatic Spell Checking (with a checkbox, currently enabled), Thesaurus… (grayed out), Language (submenu arrow), Word Count…, Accessibility Check…, and Automatic Accessibility Checking (checkbox, unchecked). The next group lists AutoCorrect (submenu arrow), AutoText…, and ImageMap (grayed out). Below that are Redact and Auto-Redact, then Heading Numbering…, Line Numbering…, and Footnote/Endnote Settings…. Further down are Mail Merge Wizard…, Bibliography Database, and Address Book Source…, followed by Update (submenu arrow) and Protect Document (submenu arrow). Calculate (grayed out) and Sort… appear next, then Macros (submenu arrow) and Development Tools (checkbox, unchecked). The final group contains XML Filter Settings…, Extensions…, Customize…, and Options… at the very bottom of the menu.
