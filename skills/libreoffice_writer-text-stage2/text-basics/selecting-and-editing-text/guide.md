# Selecting, Cutting, Copying, and Pasting Text (LibreOffice Writer 7.3.7)

Selecting text in Writer works much like any other application. You can swipe the mouse cursor over text to highlight it, or use multiple clicks for quick selections: double-click grabs a word, triple-click selects a sentence, and quadruple-click selects the entire paragraph.

For more control, click where you want to start, then press *F8* to enter **Extending selection** mode. Now just use the arrow keys to extend your selection precisely, and press *F8* again when you're done. This is especially handy when you need to highlight a large or awkward block without dragging.

You can change the selection mode from the **Selection icon** on the Status Bar at the bottom of the window. Right-click it to choose between **Standard selection**, **Extending selection** (*F8*), **Adding selection** (*Shift+F8*), and **Block selection** (*Ctrl+Shift+F8*). Left-clicking the icon cycles through them.

The Status Bar at the bottom of the Writer window shows the Selection icon as a small square button. Right-clicking it opens a pop-up menu listing four modes: Standard selection, Extending selection, Adding selection, and Block selection. In this example, Block selection is marked with a bullet indicating it is the currently active mode.

To select non-consecutive items — say, a word in one paragraph and another word three paragraphs down — use the mouse in **Standard selection** mode: select the first piece, hold *Ctrl*, then select the next piece. Repeat as needed. If you prefer the keyboard, select your first piece, press *Shift+F8* to enter **Adding selection** mode, arrow-navigate to the next piece, and hold *Shift* to extend. Press *Esc* when you're finished.

The document canvas shows a passage from "Around the World in 80 Days" by Jules Verne, with multiple non-consecutive text fragments highlighted in blue. Separate phrases such as "A puzzled grin," "Monsieur is going to leave home?," "round the world," "opened wide his eyes," "Round the world!," "Mr. Fogg," and a longer multi-line sentence near the bottom are all selected simultaneously, demonstrating how Adding selection mode allows independent pieces of text scattered across different paragraphs to be selected at once.

Need to select a vertical block of text — like a column of data that spans several lines? Use **Edit > Selection Mode > Block Area**, press *Ctrl+F8*, or click the **Selection** icon and choose **Block selection**. Then just drag or use the keyboard to highlight a rectangular region.

The document canvas shows a paragraph from "The Secret Garden" by Frances Hodgson Burnett with a rectangular block selection highlighted in blue. Rather than selecting entire lines, the highlight covers a vertical column-shaped region spanning several lines in the middle of the paragraph, selecting partial text from each line such as "a little thin body," "er face was yellow," "way or another," "ad always been busy," and "cared only to go to." This demonstrates how Block selection mode selects a rectangular area of text independent of line boundaries.

Once you've got your text selected, cutting, copying, and pasting work exactly as you'd expect. Use **Edit > Cut**, **Edit > Copy**, and **Edit > Paste** from the menu bar, or stick with the familiar keyboard shortcuts: *Ctrl+X* to cut, *Ctrl+C* to copy, and *Ctrl+V* to paste. You can move text within a document, between documents, or even between Writer and other applications — just select, cut or copy, click where you want it, and paste.

---

## UI Reference  —  Document Canvas, Rulers & Scrollbars

_Scope: Canvas click, click-drag, double-click for text selection_

The main editing surface and its surrounding controls for navigation, measurement, and layout.

## Canvas

The white rectangular area represents the printable page, surrounded by a grey pasteboard.

**Mouse interactions:**
- **Click** — Place the text cursor.
- **Click-drag** — Select a range of text.
- **Double-click** — Select the word under the cursor.
- **Right-click** — Context menu with: Paste, Clone Formatting, Clear Direct Formatting, Character (►), Paragraph (►), List (►), Insert Comment, Page Style…
- **Right-click with selection** — Adds Cut and Copy to the context menu.

The **Character** submenu offers: Character…, No Character Style, Emphasis, Strong Emphasis, Quotation, Source Text.

## Horizontal Ruler

Visible by default (toggle: View > Rulers, Shift+Ctrl+R). Displays page margins as grey bands on each end, with the white writable area between them.

**Controls:**
- **Tab-type selector** (far left button) — Click to cycle: Left, Right, Center, Decimal tab stop, First-Line Indent, Hanging Indent.
- **First-line indent** (top triangle) — Drag to set first-line paragraph indent.
- **Left indent** (bottom-left triangle) — Drag to set left indent for wrapped lines.
- **Right indent** (right triangle) — Drag to set right indent.
- **Tab stops** — Click in the white area to add; drag to move; drag off the ruler to delete.
- **Right-click** — Change measurement units: Millimeter, Centimeter, Inch, Point, Pica, Char.
- **Double-click** — Opens the Paragraph dialog.

## Vertical Ruler

Not visible by default. Enable via View > Rulers > Vertical Ruler. Shows top/bottom margins as grey bands. Right-click to change units (Millimeter, Centimeter, Inch, Point, Pica, Line).

## Scrollbars

- **Vertical scrollbar** — Right edge, for vertical document navigation.
- **Horizontal scrollbar** — Bottom edge, for horizontal navigation.
- Toggle via View > Scrollbars (both enabled by default).
