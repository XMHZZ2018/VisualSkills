...
The screenshot shows the fully expanded Insert drop-down menu from the LibreOffice Writer menu bar. It is a single-column menu listing roughly two dozen entries separated by thin dividers into logical groups: at the top are Page Break, More Breaks, Image…, Chart…, Media, OLE Object, Shape, Section…, Text from File…, Text Box, Comment, Frame, and Fontwork… (with a grayed-out Caption… entry); a middle group contains Hyperlink…, Bookmark…, and Cross-reference…; below that are Special Character…, Formatting Mark, and Horizontal Line; and the bottom group lists Footnote and Endnote, Table of Contents and Index, Page Number…, Field, Header and Footer, Envelope…, and Signature Line…. Small checkbox icons appear beside Hyperlink… and Horizontal Line, and several entries show right-pointing arrows indicating submenus.
