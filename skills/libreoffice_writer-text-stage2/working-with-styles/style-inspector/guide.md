...
The Style Inspector panel is titled "Style Inspector" at the top and contains a two-column tree with "Properties" on the left and "Values" on the right. In this example, the tree shows four expandable sections: "Paragraph Styles" (with "Text Body" listed beneath it), "Paragraph Direct Formatting" (showing "Para Keep Together" set to "true"), "Character Styles" (expanded to reveal a "Strong Emphasis" node listing attributes such as Char Font Char Set = 76, Char Font Family = 5, Char Font Name = Liberation Sans, Char Font Pitch = 2, Char Font Style Name = Regular, and Char Weight = bold), and "Character Direct Formatting" at the bottom (currently empty). A vertical strip of sidebar icon buttons is visible along the right edge beside the panel.
...
The screenshot shows the full LibreOffice Writer window with an empty document open. Along the far-right edge of the window, a narrow vertical sidebar strip is visible, containing a column of small icon buttons. The topmost button, highlighted with a tooltip reading "Properties (Alt+1)," is outlined with a red rectangle to indicate the sidebar's location. Below it are additional icons for Styles, Gallery, Navigator, Page, Style Inspector, Manage Changes, and Accessibility Check. The main document area, menu bar, and toolbars occupy the rest of the window to the left.
