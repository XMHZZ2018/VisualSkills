...
The screenshot shows the Format menu dropped down in LibreOffice Impress, displaying a vertical list of menu entries. At the top are text-related items: Text, Spacing, Align Text, Lists, Clear Direct Formatting, and Styles, followed by Character..., Paragraph..., and Bullets and Numbering... dialogs. The middle section contains Theme..., Table, Image, Text Box and Shape, a Shadow checkbox toggle, and Interaction..., Name..., and Alt Text... entries. The bottom section holds object arrangement commands: Distribute Selection, Rotate, Flip, Convert, Align Objects, Arrange, and Group — several of which show submenu arrows indicating nested options.
...
The screenshot shows the Standard toolbar as a single horizontal row of icon buttons running beneath the menu bar in LibreOffice Impress. From left to right, it begins with file operations — New, Open, and Save (each with a small dropdown arrow), followed by Email Document, Edit File toggle, a PDF export button, and Print. Next come the clipboard group (Cut, Copy, Paste) and Paint Format (clone formatting). The Undo and Redo buttons follow, then Find & Replace and the Spelling button (identified by a checkmark-over-text icon). Further right are the display view toggles for Normal, Outline, Notes, and Slide Sorter modes, plus the Presentation (Start Slideshow) button. The row ends with insertion tools — Table, Insert Chart, Insert Text Box — followed by the Hyperlink button, the Sidebar toggle, and a Start Center button at the far right.
...
_Scope: Spelling (F7), Automatic Spell Checking toggle, Language submenu, AutoCorrect Options dialog_

The Tools menu provides spelling, language, autocorrect, macros, forms, extensions, customisation, and application-wide options.

The screenshot shows the Tools menu expanded in LibreOffice Impress. At the top are the spelling-related entries: Spelling... with its F7 shortcut, Automatic Spell Checking (Shift+F7) shown with a checkmark indicating it is currently enabled, and Thesaurus... (Ctrl+F7) which appears greyed out. Below that is the Language submenu (with an arrow for nested options), followed by AutoCorrect Options.... The middle section contains Redact, Auto-Redact, and Minimize Presentation..., then checkbox-toggled panels for ImageMap, Color Replacer, and Media Player. The lower portion lists Forms and Macros submenus, a Development Tools checkbox toggle, XML Filter Settings..., Extensions... (Ctrl+Alt+E), Customize..., and finally Options... (Alt+F12) at the bottom of the menu.
