"""Phase 4 (deterministic): inline the per-region UI Reference into mm-v1.
...
Reads (all under <pipeline_dir> = output_dir from the config):
  ...
  - <pipeline_dir>/skill/images/<region>-*.png  (assembler output; cropped
    screenshots)
  - skills/<domain>-knowledge-multimodal-v1/ (mm-v1 source)

Writes:
  - skills/libreoffice_impress-knowledge-multimodal-v0/ (mm-v3, fresh copy)
    ...
    with a  ui-  prefix.  SKILL.md is unchanged from mm-v1.
...
Image references inside the appended content use the mm-v1 convention
...
logger = logging.getLogger("inline_into_mm_v1")
...
def transform_region_md(
    ...
    to glue into a mm-v1 guide.md as a "## UI Reference — <display_name>"
    ...
    rewritten to mm-v1's "Read the screenshot ..." convention, and any
...
def fresh_copy_mm_v1(src: Path, dst: Path) -> None:
    ...
    if not src.exists():
        raise SystemExit(f"mm-v1 source not found: {src}")
    if dst.exists():
        shutil.rmtree(dst)
    shutil.copytree(src, dst)
    logger.info("copied mm-v1 → %s", dst)
...
def main() -> int:
    ...
    ap.add_argument("--pipeline-dir", type=Path, required=True,
                    help="Per-domain output_dir from the v3 config; contains "
    ...
    ap.add_argument("--mm-v1", type=Path, default=None,
                    help="Source mm-v1 skill directory (default: derived from mapping)")
    ...
    if mm_v1_dir is None:
        # mm-v1 lives next to the mm-v3 target: replace -multimodal-v3 → -multimodal-v1
        target = mapping["target_skill_dir"]
        mm_v1_rel = target.replace("-multimodal-v3", "-multimodal-v1")
