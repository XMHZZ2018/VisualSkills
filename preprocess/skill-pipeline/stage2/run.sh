...
#   ./run.sh --config configs/impress.yaml --phase 5      # text-stage2
...
Phases:
  ...
  4   inline regions+images into mm-stage1 → mm-stage2   (deterministic)
  5   derive text-stage2 from mm-stage2                  (Claude API, parallel)
...
echo "[stage2] domain=$DOMAIN  config=$CONFIG_REL  phase=${PHASE:-1-3}"
...
python -u map_regions.py --pipeline-dir ${REMOTE_REPO}/${OUTPUT_DIR_REL} \
    ...
    4)
        REMOTE_CMD="cd ${REMOTE_PIPELINE} && \
python -u inline_into_stage1.py --pipeline-dir ${REMOTE_REPO}/${OUTPUT_DIR_REL} ${EXTRA_STR}"
        ;;
    5)
        REMOTE_CMD="cd ${REMOTE_PIPELINE} && \
python -u derive_text.py \
    --domain ${DOMAIN} \
    --app-name '${APP_NAME}' \
...
if [[ "${PHASE:-}" == "4" || "${PHASE:-}" == "5" ]]; then
    if [[ "$FOREGROUND" -eq 1 ]]; then
        echo "[sync-back] skills/${DOMAIN}-knowledge-{multimodal,text}-stage2/ <- VM"
        for variant in multimodal text; do
            REMOTE_SKILL="~/MMSkills/skills/${DOMAIN}-knowledge-${variant}-stage2"
            gcloud compute scp --zone="$ZONE" --recurse \
                "${VM}:${REMOTE_SKILL}" \
