...
and the mm-stage1 source skill (SKILL.md + per-topic guide.md files), then calls
`claude -p` (Opus) to emit <pipeline_dir>/region_to_guides.json matching the
schema consumed by inline_into_stage1.py.
...
def main() -> int:
    ...
    ap.add_argument("--mm-v1-dir", type=Path, default=None,
                    help="mm-stage1 source dir; default derived from --domain")
    ap.add_argument("--domain", default=None,
                    help="Used to derive mm-stage1 dir if --mm-v1-dir is not given")
    ...
    target_rel = f"skills/{args.mm_v1_dir.name.replace('-multimodal-stage1', '-multimodal-stage2')}"
