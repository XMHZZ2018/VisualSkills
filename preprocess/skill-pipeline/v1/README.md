...
> Looking for the **UI-explorer** pipeline (mm-v3 / text-v3 from live app
> exploration)? See [`../v3/README.md`](../v3/README.md). v3's Phase 5 reuses
> v1's Phase 6 derivation core (`derive_text_from_multimodal_dir`).
...
parallel:
  ...
  phase_6: 4     # text-v1 from multimodal-v1 — optional, Claude calls per guide
```
...
### Phase 6 (optional) — Text-v1 derived from multimodal-v1

Phase 6 is opt-in (run with `--phase 6`) and **not part of the default
flow**. It walks every generated multimodal-v1 guide and rewrites the
...
shows. The result is markdown-only: no images are copied into the text-v1
tree.
...
to the text-v1 cache (the topic semantics are unchanged, so the routing
keywords apply equally), and `phase_index` runs for `text` mode to write
`skills/<domain>-knowledge-text-v1/SKILL.md`.

- **Workspace cache:** `workspace/<domain>/text_v1_drafts/<cat>/<topic>.md`
- **Final output:** `skills/<domain>-knowledge-text-v1/<cat>/<topic>/guide.md`
  and `skills/<domain>-knowledge-text-v1/SKILL.md`
...
(Mode flag is ignored for phase 6 — it always reads multimodal-v1 and
writes text-v1.)
...
- **Parallelism** is set per-phase in the YAML config under a `parallel:` block:
  ...
  as phase 4), `phase_6` (text-v1 derivation, default 4 — optional phase,
