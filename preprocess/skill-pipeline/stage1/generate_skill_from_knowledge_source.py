"""Skill generation pipeline (Stage 1) from a knowledge source.
...
Pipeline:
  ...
  Phase 4 — Text skill: generate one concise guide.md per topic from the
            rendered pages. This is the text-skill-stage1 content.
  Phase 5 — Multimodal skill: pick figures relevant to the guide and splice
            ...
            *same* text, byte-for-byte identical between the two versions
            ...
            copied fig files.
  Phase 6 — Index: write SKILL.md for each -stage1 directory.
...
# This file lives at <repo>/preprocess/skill-pipeline/stage1/, so go up 3.
...
DEFAULT_PARALLEL = {
    ...
    "phase_6": 4,  # text-stage1 derivation — Claude calls per multimodal guide, same constraint
}
...
def phase_pdf_pages(
    ...
    if not pdf_path:
        print("  No PDF configured; cannot build Stage 1 skill.")
        return None, {}
...
def _draft_topic_prose(
    ...
    Used by both `generate_text_guide` (writes to skills/text-stage1/) and
    `generate_multimodal_guide` (uses in-memory when text-stage1 isn't requested).
...
def phase_use_when(
    ...
    for modality in modalities:
        ...
        if not jobs:
            print(f"  {modality}-stage1: no guides found, skipping use-when")
            continue
        ...
        print(f"  {modality}-stage1: {len(jobs)} guides ({cached} cached, {len(pending)} to compute)")
        if not pending:
            continue
...
def phase_index(config: dict, domain: str, taxonomy: dict, mode: str) -> None:
    ...
    for modality in modalities:
        ...
        print(f"  {modality}-stage1 SKILL.md updated (+ tools/skill_server.py)")
...
# ═══════════════════════════════════════════════════════════════════════════════
#
# Take each generated multimodal-stage1 guide and rewrite the inline figure
...
def _text_stage1_drafts_dir(config: dict, cat: dict) -> Path:
    return WORKSPACE_DIR / config["domain"] / "text_stage1_drafts" / cat["id"]
...
def derive_text_for_one_guide(
    ...
    Used by both Stage 1 (Phase 6) and Stage 2 (Phase 5) — see preprocess/skill-pipeline/.
...
def _derive_text_v1_for_topic(
    ...
    """Rewrite one multimodal-stage1 guide into text-stage1 prose. Stage 1-specific wrapper
    ...
    cache = _text_stage1_drafts_dir(config, cat) / f"{topic['id']}.md"
...
def derive_text_from_multimodal_dir(
    ...
    Stage 2 uses this directly. Stage 1 prefers the taxonomy-aware path
...
def phase_text_v1_from_multimodal(
    ...
    """Derive text-stage1 guides from multimodal-stage1 guides + figures."""
    ...
    if not jobs:
        print("  No multimodal-stage1 guides found — run phase 4 first.")
        return
    print(f"  {len(jobs)} multimodal guides to derive into text-stage1 (parallel={parallel})")
    ...
    with ThreadPoolExecutor(max_workers=parallel) as ex:
        ...
        for f in as_completed(futures):
            ...
            try:
                ...
                print(f"    [{topic['name']}] ERROR: {e}")
    print(
        f"  text-stage1: OK={counts['OK']} COPIED={counts['COPIED']} "
        f"SKIP={counts['SKIP']} FAILED={counts['FAILED']}"
    )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "5=use-when+index, 6=text-stage1 from multimodal [optional]). "
    ...
    if target == 6:
        ...
        print()
        print("Phase 6b: Index (text-stage1)")
