...
def run(cfg: dict, output_dir: Path) -> int:
    ...
    # Count how many actually produced notes.md
    note_counts = []
    for a in worker_args:
        notes = Path(a["output_dir"]) / "notes.md"
        note_counts.append({
            "worker_id": a["worker_id"],
            "target_id": a["target_id"],
            "notes_size": notes.stat().st_size if notes.exists() else 0,
        })
    log.info("notes.md sizes: %s", note_counts)
    usable = sum(1 for n in note_counts if n["notes_size"] > 200)
    if usable == 0:
