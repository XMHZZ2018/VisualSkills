...
IMAGE=$(read_cfg claude_cli_image cw-claude-cli)
RERUN=$(read_cfg rerun false)

echo "════════════════════════════════════════════════════════════"
echo "  cua-world evaluation runner"
echo "  Config: $CONFIG"
...
docker ps -a --filter "name=cw_" --filter "status=exited" -q 2>/dev/null | xargs -r docker rm -f 2>/dev/null || true
docker ps -a --filter "name=cw-claude-" --filter "status=exited" -q 2>/dev/null | xargs -r docker rm -f 2>/dev/null || true
# Legacy container names from before the gym-anything → cua-world rename:
...
if [[ "$RERUN" != "true" && "$RERUN" != "True" ]]; then
    RESULT_DIR=$(read_cfg result_dir scripts/run-cua-world/workspaces)
...
import yaml, sys, os
cfg = yaml.safe_load(open(sys.argv[1]))
rd = cfg.get('result_dir', 'scripts/run-cua-world/workspaces')
