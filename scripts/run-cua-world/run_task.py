"""
Run a single CUA-World task end-to-end, or perform utility operations.
...
def _run_claude_in_docker(
    ...
    container_name = f"cw-claude-{output_dir.name[:12]}-p{bridge_port}"
    try:
...
def run_single_task(cfg: dict, task_id: str, bridge_port: int) -> int:
    ...
    try:
        ...
        # Global cap: never let any task run beyond 120 steps. The higher-budget
        # tasks (Impress/Calc up to 300, Writer/QGIS up to 200) would otherwise
        # accumulate too many screenshots and hit the Anthropic 20 MB request
        # payload limit before the step budget ends. Empirically ~120 steps ≈
        # ~24 MB of PNG+prose per request; capping at 120 keeps trajectories
        # inside the payload envelope while covering the median task cleanly.
        MAX_STEPS_CAP = 120
        if max_steps and max_steps > MAX_STEPS_CAP:
            logger.info(
                "Capping max_steps %s → %s (global 120-step policy)",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run a single CUA-World task")
