"""
HTTP bridge that sits between our MCP server and CUA-World's env API.
...
that the cua-world-controller MCP server calls.

Action format translation:
    MCP server sends high-level actions → bridge translates to CUA-World's
    internal format:
...
def _translate_action(body: dict) -> list[dict]:
    """Translate MCP-level action dict into CUA-World internal format.

    cua-world env.step() expects a list of dicts in the format:
        {"mouse": {"left_click": [x, y]}}
        {"keyboard": {"text": "hello"}}
        {"keyboard": {"keys": ["ctrl", "a"]}}
    etc.
    ...
    if action_type == "left_click":
        ...
        # cua-world: positive = down, negative = up
        ...
        # cua-world expects keys as a list: ["ctrl", "a"]
...
class BridgeHandler(BaseHTTPRequestHandler):
    ...
    def _handle_execute_action(self, body: dict) -> None:
        """Translate MCP action and execute on cua-world env."""
        bridge = self._bridge
        with bridge.env_lock:
