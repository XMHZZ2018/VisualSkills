# Setup Fix Guide for gym-anything Environments

When adding a new software domain to gym-anything, the per-task `setup_task.sh` scripts often have common bugs that prevent Claude from starting work immediately. This guide documents the issues we found and fixed, so you can proactively check for them in any domain.
